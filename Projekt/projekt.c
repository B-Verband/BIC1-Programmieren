...
void menu(node_t *hashtable[], int size)
{
	...
	while (option < 1 || option > 5)
	{
		...
		case 1:
			/* Eingabe */
			printf("Neu Matrikelnummer eingeben: "); scanf("%d", &matrikelnummer);
			printf("Namen eingeben: "); scanf("%s", nachname);
			/* Hinzufügen */
			status = insert(hashtable, size, matrikelnummer, nachname);
			/* Ausgabe */
			if (status == INSERTED)				printf("StudentIn eingefuegt.\n");
			else if (status == ALREADY_EXISTS)	printf("Matrikelnummer existiert bereits.\n");
			break;
		case 2:
			/* Eingabe */
			...
			/* Ausgabe */
			if (status == DELETED)			printf("StudentIn geloescht.\n");
			else if (status == NOT_FOUND)	printf("Matrikelnummer existiert nicht.\n");
			break;
		case 3:
			/* Eingabe */
			...
			/* Ausgabe */
			if (status == FOUND)			printf("Student: %s\n", nachname);
			else if (status == NOT_FOUND)	printf("Matrikelnummer existiert nicht.\n");
			break;
		case 4:
